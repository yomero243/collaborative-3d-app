...
const HolographicShaderType = shaderMaterial(
  {
    time: 0,
    fresnelOpacity: 1.0,
    fresnelAmount: 0.45,
...
export default function HolographicMaterial(props: HolographicMaterialProps) {
  ...
  const materialRef = useRef<InstanceType<typeof HolographicShaderType> & { time: number }>(null);
  ...
  const materialProps = {
    ...
    hologramBrightness,
    signalSpeed,
    hologramColor: hologramColor ? new Color(hologramColor) : undefined,
    enableBlinking,
    blinkFresnelOnly,
    hologramOpacity,
  };

  const filteredProps = Object.fromEntries(Object.entries(materialProps).filter(([, v]) => v !== undefined));

  return (
    <holographicShaderType
      ref={materialRef}
      key={HolographicShaderType.key}
      side={
        side === 'DoubleSide'
          ? DoubleSide
          : side === 'BackSide'
